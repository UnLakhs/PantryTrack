import { addFoodItem, updateFoodItem } from "../api/food-items";
...
const FoodItemFormModal = ({
  ...
  const isEditing = itemToEdit !== null && itemToEdit !== undefined;
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const foodItem: FoodItemRequest = {
      ...
      storageLocation: formData.get(
        "storageLocation",
      ) as FoodItemRequest["storageLocation"],
      notes: String(formData.get("notes")),
    };

    try {
      if (isEditing && itemToEdit) {
        await updateFoodItem(itemToEdit.id, foodItem);
      } else {
      ...
      setErrorMessage(
        "Could not save food item. Check the fields and try again.",
      );
    } finally {
        ...
        <div className="flex flex-col gap-2 border-b border-slate-200 pb-5 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              {isEditing
                ? "Update the details of an existing food item."
                : "Create a new item in your pantry inventory."}
            </p>
          </div>
          ...
          <button
            type="button"
            onClick={onClose}
            className="self-start rounded-md border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700 shadow-sm transition hover:bg-slate-50 hover:cursor-pointer"
          >
            Close
          </button>
        </div>
        ...
        >
          ...
          <label className="block">
            ...
            <select
              ...
              defaultValue={itemToEdit?.storageLocation || "PANTRY"}
              className="mt-2 h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-sm outline-none transition focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100"
            >
              <option className="hover:cursor-pointer" value="FRIDGE">Fridge</option>
              <option className="hover:cursor-pointer" value="FREEZER">Freezer</option>
              <option className="hover:cursor-pointer" value="PANTRY">Pantry</option>
            </select>
          </label>
